...
class MainBinanceFutures extends BinanceTools {
	...
	async MultiUserTrade(Signal, CurrencyPrice) {
		const UsersPack = await User.GetTradeUsers('Futures', Signal.Currency[1]);
		...
		UsersPack.map((UserList, UserIndex) => {
			setTimeout(() => {
				UserList.map(async (UserItem) => {
					...
					try {
						/* ---------------------------- Balance Check ---------------------------- */
						let AssetBalance = await this.FuturesCheckBalance(BinanceConnection);
						...
						if (Signal.Currency[1] === 'USDT') {
							const AmountRequired = parseFloat(
								Signal.Capital.Percentage *
									parseInt(UserItem.ExchangesData.Binance.FuturesCeilingAmount.USDT) /
									100
							);
							...
							if (AmountRequired > parseFloat(AssetBalance)) {
								...
								console.log(
									await this.SpotToFutures(Math.ceil(Difference), BinanceConnection),
									'SpotToFutures',
									Difference,
						...
						const Quantity = await this.CalculationQuantity(
							{
								USDT_BALANCE_FUTURES: UserItem.ExchangesData.Binance.FuturesCeilingAmount.USDT
							},
							Signal.Capital.Percentage,
							Currency,
							parseFloat(CurrencyPrice),
							Signal.ExchangeType[0],
							Signal.Capital.Range[Signal.Capital.Range.length - 1],
							Signal.Currency[1]
						);
						...
						console.log(CurrencyPrice, 'CurrencyPrice', UserItem.Username, Signal.ChatID);

						console.log(Quantity, 'Quantity', UserItem.Username, Signal.ChatID);

						console.log(MainPositionStatus, 'MainPositionStatus', UserItem.Username, Signal.ChatID);

						if (MainPositionStatus) {
							var PositionOrderID = MainPositionStatus.orderId;
							if (typeof MainPositionStatus.orderId === 'object') {
	...
	async SpotToFutures(Amount, BinanceConnection) {
		let TranStatus = await BinanceConnection.transferMainToFutures('USDT', Amount);
	...
	CheckPendingPositions(Prices) {
		...
		Prices.forEach((SymbolItem) => {
			if (SymbolItem.eventType === 'markPriceUpdate') PricesFilter[SymbolItem.symbol] = SymbolItem.markPrice;
		});
		...
		Object.keys(this.PendingPositions.Opening).map((SignalCurrency) => {
			this.PendingPositions.Opening[SignalCurrency].map(async (Signal, SignalIndex) => {
				...
				if (InRange) {
					...
					await this.MultiUserTrade(Signal, PricesFilter[SignalCurrency]);
	...
	async Trade(Signal) {
		// Wrong signal
		if (Signal.ExchangeType[0] != 'binance-futures' || [ 'long', 'short' ].indexOf(Signal.ExchangeType[1]) === -1) {
	...
	async SetMarginType(Currency, MarginType, BinanceConnection) {
		...
		await BinanceConnection.futuresMarginType(Currency, MarginType.toUpperCase());
	}
	...
	async MarketLongPositionRM(Currency, Quantity, MarginType, Leverage) {
		...
		return await global.BinanceFutures.BinanceConnection.futuresMarketBuy(Currency, Quantity);
	}

	async MarketLongPosition(Currency, Quantity, MarginType, Leverage, ChatID, BinanceConnection) {
		await this.SetMarginType(Currency, MarginType, BinanceConnection);
		await this.SetLeverage(Currency, Leverage, BinanceConnection);

		return await BinanceConnection.futuresMarketBuy(Currency, this.ScientificToDecimal(Quantity));
	}
	...
	async MarketShortPosition(Currency, Quantity, MarginType, Leverage, ChatID, BinanceConnection) {
		await this.SetMarginType(Currency, MarginType, BinanceConnection);
		await this.SetLeverage(Currency, Leverage, BinanceConnection);

		return await BinanceConnection.futuresMarketSell(Currency, this.ScientificToDecimal(Quantity));
	}

	async OrderLongPosition(Currency, Quantity, OpenOrderPrice, Leverage, MarginType, Last, BinanceConnection) {
		// if (!global.BinanceFutures.BinanceConnection) this.CreateBinanceConnection();
		try {
			var Options = {
				positionSide: 'SHORT',
			...
			if (Last) {
				...
				Options.price = this.ScientificToDecimal(this.ToFixed(OpenOrderPrice, SymbolData[Currency].TickSize));
				Options.type = 'LIMIT';
			}
	...
	async OrderShortPosition(Currency, Quantity, OpenOrderPrice, Leverage, MarginType, Last, BinanceConnection) {
		try {
			var Options = {
				positionSide: 'LONG',
				side: 'SELL'
	...
	async AddSL(Currency, SignalSide, Quantity, StopPrice, ChatID, BinanceConnection, Username) {
		let OrderStatus = {};
		var StoplossItem = [];
		...
		if (SignalSide === 'long') {
			OrderStatus = await BinanceConnection.futuresSell(Currency, 0, 0, {
				...
				timeInForce: 'GTE_GTC',
				type: 'STOP_MARKET',
				stopPrice: this.ScientificToDecimal(this.ToFixed(StopPrice, SymbolData[Currency].PricePrecision)),
		...
		setTimeout(() => {
			global.FuturesPositionChecker.AddOrderToMonitorList(Username, ChatID, [], StoplossItem, Currency);
		}, 20000);
	...
	AddTP(Currency, SignalSide, Quantity, Prices, MarginType, Leverage, ChatID, BinanceConnection, Username) {
		...
		while (!OverCurrencyQuantity) {
			console.log(SymbolData[Currency].QuantityPrecision, 'SymbolData[Currency].QuantityPrecision', Username);

			TargetsQuantity = this.ToFixed(
				Quantity / (Prices.length - TargetCount),
				SymbolData[Currency].QuantityPrecision
			);
			if (TargetsQuantity >= SymbolData[Currency].MinQuantity) OverCurrencyQuantity = true;
			else TargetCount++;
		}

		var NewTargetsQuantity = this.ToFixed(TargetsQuantity, SymbolData[Currency].QuantityPrecision);

		console.log(NewTargetsQuantity, 'NewTargetsQuantityNewTargetsQuantityNewTargetsQuantity');
		...
		Prices.map(async (Price, PriceIndex) => {
			...
			if (!PriceIndex)
				LocalQuantity = this.ToFixed(
					parseFloat(NewTargetsQuantity) + parseFloat(RemainingQuantity),
					SymbolData[Currency].QuantityPrecision
				);
			...
			if (Prices.length - 1 === PriceIndex) {
				...
				setTimeout(() => {
					global.FuturesPositionChecker.AddOrderToMonitorList(Username, ChatID, TakeProfitItem, [], Currency);
				}, 20000);
			}
		});
	}
	...
	FuturesMarkPriceStream() {
		global.BinanceFutures.BinanceConnection.futuresMarkPriceStream(this.CheckPendingPositions);
	}
}
