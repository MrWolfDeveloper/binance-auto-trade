...
class SpotPositionChecker {
	...
	async StopLossHited(OrderUUID) {
		try {
			const { 0: Username, 1: ChatID, 2: OrderID, 3: OrderType } = OrderUUID.split(';');
			...
			setTimeout(() => {
				this.PendingOrderUUIDList = this.PendingOrderUUIDList.filter(function(Item) {
			...
			if (OrderType === 'oco_order') {
				...
				const OrderUUIDRegex = new RegExp(`${Username};${ChatID};(.*)`, 'g');
				...
				// Remove stoploss
				var RemoveIndex = global.SpotMonitorList.StopLoss
					.map((Order, OrderIndex) => {
						if (Order.match(OrderUUIDRegex)) return OrderIndex;
					})
					.filter(function(Element) {
				...
				// Sort done targets
				var DoneTargets = UserSignal.DoneTargets.sort((a, b) => parseFloat(a.Price) - parseFloat(b.Price));
						...
						ChatID,
						{
							$push: {
								'Signals.$.DoneTargets': {
									...
									TargetCreationDate: DTItem.TargetCreationDate,
	...
	async TakeProfitHited(OrderUUID) {
		...
		if (OrderType === 'oco_order') {
			...
			// Which target hited
			var HitedIndex = DoneTargets.findIndex((DT) => OrderID.toString() === DT.TPOrderID.toString());
			...
			if (HitedIndex === DoneTargets.length - 1) {
				...
				setTimeout(async () => {
					DoneTargets.map(async (NewTarget) => {
						...
						if (NewTarget.Hit) {
							if (!NewTarget.HitDate) DN.HitDate = new Date(Date.now());
						}
				...
				if (HitedIndex >= 1) {
					...
					this.CancelAllOCOOrders(SignalCurrency, UserSignal.DoneTargets, BinanceConnection);
				}
					...
					DoneTargets.map(async (NewTarget) => {
						if (HitedIndex >= 1) {
							this.OrderOCO(
								...
								SignalCurrency,
								this.ToFixed(NewTarget.OrderQuantity, SpotSymbolData[SignalCurrency].QuantityPrecision),
								NewTarget.Price,
								StopLoss,
								ChatID,
								NewTarget.Hit,
			...
			User.Signals.map((Signal) => {
				...
				for (var i = 0; i < Signal.DoneTargets.length; i++) {
					if (Signal.DoneTargets[i].Type === 'oco_order') {
						OrderType = 'oco_order';
						if (typeof Signal.DoneTargets[i].Hit === 'undefined' || !Signal.DoneTargets[i].Hit) {
	...
	AddOrderToMonitorList(Currency, Username, ChatID, TakeProfits, StopLoss, OrderType) {
		// Ready take profits
		var NewTP = TakeProfits.map((OrderID) => {
			var OrderUUIDGen = `${Username};${ChatID};${OrderID};${OrderType}`;

			if (global.SpotMonitorList.TakeProfit.indexOf(OrderUUIDGen) > -1) return null;
		...
		var NewSL = StopLoss.map((OrderID) => {
			...
			if (global.SpotMonitorList.StopLoss.indexOf(OrderUUIDGen) > -1) return null;
			else return OrderUUIDGen;
		});
		...
		// Add to list
		global.SpotMonitorList.TakeProfit = [ ...global.SpotMonitorList.TakeProfit, ...NewTP ];
		global.SpotMonitorList.StopLoss = [ ...global.SpotMonitorList.StopLoss, ...NewSL ];

		console.log(global.SpotMonitorList, 'global.SpotMonitorList');

		// Add currency to monitor list
		if (this.CurrencyListMonitor.indexOf(Currency) === -1) this.CurrencyListMonitor.push(Currency);
	...
	AddSpotTradeWS() {
		...
		if (this.LastTradeWSID) {
			global.BinanceSpot.BinanceConnection.websockets.terminate(this.LastTradeWSID);
		}
		...
		this.LastTradeWSID = global.BinanceSpot.BinanceConnection.websockets.trades(
			this.CurrencyListMonitor,
			(Trades) => {
				...
				var TakeProfitHitedIndex = global.SpotMonitorList.TakeProfit.findIndex(
					(TP) => TradeID.toString() === TP.split(';')[2]
				);

				if (TakeProfitHitedIndex > -1) {
					return this.TakeProfitHited(global.SpotMonitorList.TakeProfit[TakeProfitHitedIndex]);
				}

				var StopLossHitedIndex = global.SpotMonitorList.StopLoss.findIndex(
					(SL) => TradeID.toString() === SL.split(';')[2]
				);

				if (StopLossHitedIndex > -1) {
					return this.StopLossHited(global.SpotMonitorList.StopLoss[StopLossHitedIndex]);
				}
			}
		);
	...
	RenewTradeWS() {
		...
		setInterval(() => {
			...
			if (Self.LastTradeWSID) {
				global.BinanceSpot.BinanceConnection.websockets.terminate(Self.LastTradeWSID);
			}

			Self.LastTradeWSID = global.BinanceSpot.BinanceConnection.websockets.trades(
				...
				(Trades) => {
					...
					if (TakeProfitHitedIndex > -1) {
						return Self.TakeProfitHited(global.SpotMonitorList.TakeProfit[TakeProfitHitedIndex]);
					}
					...
					if (StopLossHitedIndex > -1) {
						return Self.StopLossHited(global.SpotMonitorList.StopLoss[StopLossHitedIndex]);
					}
				}
			);
			// }, 60000);
	...
	OCOStatusAnalysis(SLData, TPData) {
		if (this.APIHitStatus.indexOf(SLData.status) > -1 && TPData.status === 'EXPIRED') return 'stoploss';
		else if (this.APIHitStatus.indexOf(TPData.status) > -1 && SLData.status === 'EXPIRED') return 'take_profit';
	...
	CancelAllOCOOrders(Currency, DoneTargets, BinanceConnection) {
		DoneTargets.map(async (Target) => {
			...
			try {
				await this.CancelOrder(Currency, Target.SLOrderID, BinanceConnection);
			} catch (ReturnError) {
	...
	async OrderOCO(
		...
		if (Currency.indexOf('USDT') > -1) {
			StopPriceLimit = parseFloat(parseFloat(StopLoss) + parseFloat(StopLoss) * 0.0001);
		} else {
			StopPriceLimit = this.ToFixed(
				parseFloat(parseFloat(StopLoss) + parseFloat(StopLoss) * 0.0001),
			...
			if (!Hit) {
				OrderStatus = await BinanceConnection.sell(
					...
					this.ScientificToDecimal(parseFloat(Price)),
					{
						...
						quantity: this.ScientificToDecimal(parseFloat(Quantity)),
						side: 'SELL',
						stopLimitPrice: this.ScientificToDecimal(parseFloat(StopLoss)),
				...
				if (typeof HitDate === 'undefined') DN.HitDate = new Date(Date.now());
	...
	FindPositionNumber(DoneTargets, Targets, Price) {
		try {
			...
			if (SortableTargets.indexOf(parseFloat(Price)) > -1) {
				...
				DoneTargets = DoneTargets.sort((a, b) => parseFloat(a.Price) - parseFloat(b.Price));
				...
				return { TargetNumber: HitedIndex, NewDoneTargets: DoneTargets };
			} else {
				return 'error';
			}
		} catch (ReturnError) {
			...
			try {
				...
				for (var i = 0; i < Signal.DoneTargets.length; i++) {
					if (Signal.StopLoss.Type === 'manual') {
						...
						const OrderStatus = await this.OrderStatus(Currency, Signal.DoneTargets[i].OrderID);

						// Take profit hited
						if (this.APIHitStatus.indexOf(OrderStatus.status) > -1) {
							// Update DoneTarget
							await DataBaseQuery.AsyncMakeDatabaseQuery({
								...
								MethodData: {
									...
									Which: {
										...
										'DoneTargets.Price': Signal.DoneTargets[i].Price.toString()
									}
								}
							});
							...
							SignalUpdate.SignalUpdate(Signal.ChatID, {
								$set: {
									Quantity:
										parseFloat(Signal.Quantity) - parseFloat(Signal.DoneTargets[i].OrderQuantity),
						...
						if (!Signal.DoneTargets[i].Hit) {
							const OCOStatus = this.OCOStatusAnalysis(
								await this.OrderStatus(Currency, Signal.DoneTargets[i].SLOrderID),
								await this.OrderStatus(Currency, Signal.DoneTargets[i].TPOrderID)
							);
							...
							if (OCOStatus != 'not_hited') {
								...
								this.CancelAllOCOOrders(Currency, Signal.DoneTargets);
								...
								if (OCOStatus === 'take_profit') {
									...
									if (NewData.TargetNumber != 'take_profit') {
										PositionStatus = `target-${NewData.TargetNumber + 1}`;

										var StopLoss;
										if (NewData.TargetNumber === 0) {
											StopLoss = Signal.EntryPrice;
										} else {
											StopLoss = Signal.DoneTargets[NewData.TargetNumber - 1].Price;
										}
										...
										setTimeout(async () => {
											NewData.NewDoneTargets.map(async (NewTarget) => {
												this.OrderOCO(
													NewTarget.TargetCreationDate,
													Currency,
													this.ToFixed(
