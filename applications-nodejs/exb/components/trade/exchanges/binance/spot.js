...
class MainBinanceSpot extends BinanceTools {
    ...
    constructor() {
        ...
        this.API_KEY =
            '5LmRj72iyRBlN7JRG834NkW0RHmr5OMeMiOSa4mLHrIAYM3LVVv5OOZBDGDZZg9K';
        this.API_SECRET =
        ...
        UsersPack.map((UserList, UserIndex) => {
            setTimeout(() => {
                UserList.map(async (UserItem) => {
                    ...
                    try {
                        /* ---------------------------- Balance Check ---------------------------- */
                        let AssetBalance = await this.SpotBalanceCheck(
                            Signal.Currency[1],
                            BinanceConnection
                        );
    ...
    async FuturesToSpot(Asset, Amount, BinanceConnection) {
        let TranStatus = await BinanceConnection.transferFuturesToMain(
            Asset,
            Amount
        );
    ...
    async Market(Currency, Quantity, ChatID, BinanceConnection) {
        try {
            const OrderStatus = await BinanceConnection.marketBuy(
                Currency,
    ...
    AddTP(
        ...
        while (!OverCurrencyQuantity) {
            ...
            if (StopLoss * TargetsQuantity >= MinimumOrder)
        ...
        Prices.map(async (Price, PriceIndex) => {
            ...
            if (!PriceIndex)
                LocalQuantity =
                    ...
                    parseFloat(OtherRemainingQuantity);
            ...
            let OrderStatus = await this.OrderOCO(
                Currency,
                LocalQuantity,
                Price,
                StopLoss,
    ...
    AddTPCandle(
        ...
        Prices.map(async (Price, PriceIndex) => {
            ...
            var LocalQuantity = this.ToFixed(
                LocalQuantity,
                SpotSymbolData[Currency].QuantityPrecision
            );
            ...
            let OrderStatus = await this.Order(
                Currency,
                LocalQuantity,
                Price,
                BinanceConnection
    ...
    async CancelMarketSignal(Currency, Quantity, BinanceConnection) {
        try {
            const OrderStatus = await BinanceConnection.marketSell(
                Currency,
                this.ScientificToDecimal(Quantity)
            );
        ...
        AllUsers[1].map((UserItem) => {
            UserItem.Signals.map(async (UserSignal) => {
                if (
                    ...
                    UserSignal.PositionStatus.match(
                        /(?:^|W)(open|target-(.*))(?:$|W)/i
                    )
                ) {
                    ...
                    for (var i = RemoveIndex.length - 1; i >= 0; i--)
                        global.SpotMonitorList.TakeProfit.splice(
                            RemoveIndex[i],
                            1
                        );
