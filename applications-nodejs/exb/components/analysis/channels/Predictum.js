...
Predictum = {
    Precision: (ExchangeType, Currency, Amount) => {
        ...
        if (ExchangeType === 'binance-spot')
            return SpotPricePrecision[Currency].PricePrecision;
    ...
    AnalysisContent: (FilesContent) => {
        var AnalysisContent = FilesContent.map((ChannelData) => {
            return ChannelData.Data.map((Content, index) => {
                ...
                const RegexFileBK =
                    require(`${__dirname}/../regex/bk.js`).Regex;
                ...
                var ExchangeType = Content.message.match(
                    RegexFile.ExchangeType
                );
                ...
                if (ExchangeType) {
                    ...
                    if (ExchangeType[0] === 'binance-futures') {
                        if (ExchangeType[1] === 'long')
                            ReductionCoefficient = 0.999;
                        else if (ExchangeType[1] === 'short')
                            ReductionCoefficient = 1.001;
                    } else {
                        ReductionCoefficient = 0.999;
                    }

                    if (
                        NewCapital.indexOf('(') > -1 &&
                        ExchangeType[0] === 'binance-futures'
                    ) {
                        var CapitalText = NewCapital.match(
                            /((?:isolated|cross)(.*))/gm
                        );

                        CapitalText = CapitalText[0]
                            .replace(')', '')
                            .replace('x', '');
                        ...
                        CapitalOBJ.Leverage =
                            CapitalText.match(/((?:isolated|cross))/gm)[0];
                        CapitalOBJ.Range = CapitalText.replace(
                            /((?:isolated|cross))/gm,
                            ''
                        ).split('-');
                    }
                    ...
                    StopLoss.indexOf('manual') > -1
                        ? (StopLossOBJ.Type = 'manual')
                        : (StopLossOBJ.Type = 'normal');

                    if (StopLossOBJ.Type === 'manual') {
                        if (StopLoss.indexOf('daily') > -1)
                            StopLossOBJ.CandleInterval = '1d';
                        if (StopLoss.indexOf('weekly') > -1)
                            StopLossOBJ.CandleInterval = '1w';
                        if (StopLoss.indexOf('1h') > -1)
                            StopLossOBJ.CandleInterval = '1h';
                        if (StopLoss.indexOf('4h') > -1)
                            StopLossOBJ.CandleInterval = '4h';
                        if (StopLoss.indexOf('1m') > -1)
                            StopLossOBJ.CandleInterval = '1m';

                        StopLossOBJ.Number = StopLoss.match(
                            RegexFile.ManualStopLoss
                        );
                        StopLossOBJ.Number = StopLossOBJ.Number[0]
                            .replace(/(?:below|above)/i, '')
                            .trim();
                    } else {
                        StopLossOBJ.Number = StopLoss.match(
                            RegexFile.NormalStopLoss
                        );
                        StopLossOBJ.Number = StopLossOBJ.Number[0].replace(
                            'normalstoploss:',
                            ''
                        );
                        ...
                        StopLossOBJ.Number = parseFloat(
                            StopLossOBJ.Number
                        ).toFixed(StopLossPrecision);
                    }
                    ...
                    var NewTargets = Targets.map((Target) =>
                        Target.replace('Target:', '').trim()
                    );
                    ...
                    var StructuredTargets = NewTargets.map((Target) => {
                        ...
                        return parseFloat(TargetNumber).toFixed(
                            TargetPrecision
                        );
                    });
                    ...
                    var StructuredOpenTargets = OpenTargets.map(
                        (OpenTarget) => {
                            ...
                            OpenTargetNumber =
                                OpenTargetNumber * ReductionCoefficient;

                            return parseFloat(OpenTargetNumber).toFixed(
                                OpenTargetPrecision
                            );
                        }
                    );
                    ...
                    var Direction = Content.message.match(
                        RegexFileBK.ExchangeType
                    );
                    var Currency = Content.message.match(RegexFileBK.Currency);
                    var EnterPrice = Content.message.match(
                        RegexFileBK.EnterPrice
                    );
                    ...
                    if (CapitalOBJ.Range.toString() === '1')
                        CapitalOBJ.Range = '2';
                    ...
                    StopLossOBJ.Number = StopLoss.replace(/,/gm, '').replace(
                        'stoploss:',
                        ''
                    );
                    ...
                    // StopLossOBJ.Number = StopLossOBJ.Number * ReductionCoefficient;
                    StopLossOBJ.Number = parseFloat(StopLossOBJ.Number).toFixed(
                        StopLossPrecision
                    );
                    ...
                    // Remove empty targets
                    var NewTargets = Targets[0].map((Target) =>
                        Target.replace('Target:', '').trim()
                    );
                    NewTargets = NewTargets.filter((Item) => Item);
