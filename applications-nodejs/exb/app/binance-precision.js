...
const SaveData = (FileName, SaveData) => {
	FS.writeFileSync(`${__dirname}/../../../storage/exchange-data/${FileName}.json`, JSON.stringify(SaveData));
};
...
const SpotGetExchangeInfo = () => {
	return new Promise((Resolve, Reject) => {
		Request('https://api.binance.com/api/v3/exchangeInfo', (ReturnError, Response, Body) => {
...
async function SpotOperation() {
	try {
		const SpotStructuredExchangeInfo = SpotStructureData(await SpotGetExchangeInfo());
...
async function FuturesOperation() {
	try {
		const FuturesStructuredExchangeInfo = await FuturesStructureData(await GetExchangeInfoFutures());
